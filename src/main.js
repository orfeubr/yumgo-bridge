...
autoUpdater.autoDownload = false; // Nunca baixar automaticamente - sempre perguntar
...
// Flag para distinguir check automatico vs manual
...
autoUpdater.on('checking-for-update', () => {
    ...
    log.info(`🌐 Feed URL: https://github.com/orfeubr/yumgo-bridge/releases`);
});
...
autoUpdater.on('update-available', (info) => {
    ...
    // 1. Notificação nativa do sistema (aparece mesmo com app minimizado)
    showNotification(
        '🔄 Atualização Disponível!',
        `Nova versão ${info.version} do YumGo Bridge. Clique para atualizar.`
    );

    // 2. Trazer janela para frente
    if (mainWindow) {
        if (mainWindow.isMinimized()) mainWindow.restore();
        mainWindow.show();
        mainWindow.focus();

        // 3. Avisar a UI para mostrar banner de atualização
    ...
    // 4. Dialog pedindo para baixar
    dialog.showMessageBox(mainWindow || null, {
        type: 'info',
        title: 'Atualização Disponível',
        message: `Nova versão ${info.version} disponível!`,
        detail: `Você está usando a versão ${app.getVersion()}.\n\nRecomendamos atualizar para ter as últimas melhorias e correções.\n\nDeseja baixar agora?`,
        ...
        if (result.response === 0) {
            ...
            setTimeout(() => {
                log.info('🔍 Re-verificando atualização (lembrete)...');
...
autoUpdater.on('update-not-available', (info) => {
    ...
    // So mostrar mensagem se foi verificacao manual
...
autoUpdater.on('error', (err) => {
    log.error('❌ Erro ao verificar atualização:', err.message);

    // So mostrar dialog se foi verificacao manual (via menu)
    if (isManualUpdateCheck && mainWindow) {
        const errMsg = (err.message || '').toLowerCase();
        let detail = err.message;

        if (errMsg.includes('network') || errMsg.includes('enotfound')) {
            detail = 'Sem conexão com a internet. Tente novamente mais tarde.';
        } else if (errMsg.includes('404')) {
            detail = 'Servidor de atualizações indisponível. Tente novamente mais tarde.';
        }
...
function checkForUpdates() {
    ...
    log.info('═══════════════════════════════════════════════════════');
    log.info('🔎 VERIFICAÇÃO DE ATUALIZAÇÃO INICIADA (manual)');
    log.info(`📌 Versão instalada: ${app.getVersion()}`);
    log.info(`🌐 Repositório: orfeubr/yumgo-bridge`);
    log.info('═══════════════════════════════════════════════════════');
...
function connectWebSocket(restaurantId, token) {
    ...
    try {
        ...
        channel.bind('.order.created', async (data) => {
            ...
            log.info(`🔍 DEBUG: Total de itens recebidos: ${data.order.items?.length || 0}`);

            // DEBUG: Listar todos os itens recebidos
            if (data.order.items && data.order.items.length > 0) {
                data.order.items.forEach((item, index) => {
                    log.info(`  Item ${index + 1}: ${item.quantity}x ${item.name}`);
                    if (item.variations) log.info(`    Variações: ${JSON.stringify(item.variations)}`);
                    if (item.addons) log.info(`    Adicionais: ${JSON.stringify(item.addons)}`);
                    if (item.notes) log.info(`    Obs: ${item.notes}`);
                });
            } else {
                log.error(`❌ ERRO: Pedido sem itens!`);
                log.error(`📦 Payload completo: ${JSON.stringify(data.order, null, 2)}`);
            }
...
app.whenReady().then(() => {
    ...
    if (!isDev) {
        // Verificar 5s após iniciar
        setTimeout(() => {
            log.info('🔍 Verificando atualizações automaticamente...');
            autoUpdater.checkForUpdates().catch(err => log.error('Erro ao verificar:', err.message));
        }, 5000);

        // Verificar a cada 4 horas
        setInterval(() => {
            log.info('🔍 Verificação periódica de atualizações...');
            autoUpdater.checkForUpdates().catch(err => log.error('Erro ao verificar:', err.message));
        }, 4 * 60 * 60 * 1000);
    }
