@php
    ...
    if ($percentage >= 100) {
        ...
        $message = 'Você atingiu o limite de produtos do plano ' . $planName . '. Faça upgrade para adicionar mais produtos.';
    } elseif ($percentage >= 90) {
        $colorClass = 'bg-orange-50 border-orange-300';
        $iconColor = 'text-orange-600';
        $progressColor = 'bg-orange-500';
        $textColor = 'text-orange-900';
        $title = '⚠️ Limite de produtos quase atingido';
        $message = 'Você está usando ' . $percentage . '% do limite de produtos. Considere fazer upgrade antes de atingir o limite.';
    } elseif ($percentage >= 70) {
        $colorClass = 'bg-yellow-50 border-yellow-300';
        $iconColor = 'text-yellow-600';
        $progressColor = 'bg-yellow-500';
        $textColor = 'text-yellow-900';
        $title = '💡 Aproximando do limite de produtos';
        $message = 'Você está usando ' . $percentage . '% do limite de produtos do plano ' . $planName . '.';
    } else {
        $colorClass = 'bg-blue-50 border-blue-300';
        $iconColor = 'text-blue-600';
        $progressColor = 'bg-blue-500';
        $textColor = 'text-blue-900';
        $title = '✅ Produtos dentro do limite';
        $message = 'Você está usando ' . $current . ' de ' . $limit . ' produtos disponíveis no plano ' . $planName . '.';
    }
@endphp
            ...
            <div class="flex-1">
                ...
                <div class="mb-3">
                    <div class="flex justify-between text-xs {{ $textColor }} mb-1">
                        <span>{{ $current }} / {{ $limit }} produtos</span>
